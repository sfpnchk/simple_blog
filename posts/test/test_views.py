...
class HomePageTest(TestCase):
    ...
    def setUpTestData(cls):
        activated_user = User.objects.create(
            email="test1@test.com", password="test_pass2", is_active=True
        )

        published_Posts = Posts.objects.create(
            content="PUBLISHED posts by activated user",
            title="test",
            author=activated_user,
            status=Posts.PostStatus.PUBLISHED,
        )
        unpublished_Posts = Posts.objects.create(
            content="UNPUBLISHED posts by activated user",
            title="test",
            author=activated_user,
            status=Posts.PostStatus.UNPUBLISHED,
        )
        waiting_confirmation_Posts = Posts.objects.create(
            content="WAITING_CONFIRMATION posts by activated user",
            title="test",
            author=activated_user,
    ...
    def test_unpublished_posts_from_activated_user_not_show(self):
        ...
        self.assertNotContains(
            resp, "UNPUBLISHED posts by activated user", status_code=200
        )

    def test_wait_confirmation_posts_from_activated_user_not_show(self):
        resp = self.client.get(reverse("posts:main"))
        self.assertNotContains(
            resp, "WAITING_CONFIRMATION posts by activated user", status_code=200
        )
    ...
    def test_pagination_on_15_posts_show(self):
        activated_user = User.objects.create(
            email="testpagination@test.com", password="test_pass2", is_active=True
        )

        for i in range(15):
            posts = Posts.objects.create(
                content=f"PUBLISHED posts {i} by activated user",
                title="test",
                author=activated_user,
                status=Posts.PostStatus.PUBLISHED,
            )
...
class PostsDetailTest(TestCase):
    def setUp(self):
        ...
        self.published_posts = Posts.objects.create(
            content="PUBLISHED posts by activated user",
            title="test",
            author=activated_user,
            status=Posts.PostStatus.PUBLISHED,
        ...
        self.unpublished_posts = Posts.objects.create(
            content="PUBLISHED WAITING_CONFIRMATION posts by activated user",
            title="test",
            author=activated_user,
            status=Posts.PostStatus.WAITING_CONFIRMATION,
    ...
    def test_detail_page_status_code(self):
        resp = self.client.get(
            reverse("posts:detail", kwargs={"pk": self.published_posts.id})
        )
        self.assertEquals(resp.status_code, 200)
...
class PostsCreateTest(TestCase):
    def setUp(self):
        user = User.objects.create(
            email="testcreate@test.com", password="test_pass2", is_active=True
        )
        posts = Posts.objects.create(
            content="PUBLISHED posts by activated user",
            title="test",
    ...
    def test_creation_posts_by_authorized_user(self):
        resp = self.client.post(
            reverse("posts:create"), {"title": "test", "content": "testcontent"}
        )
...
class PostsDeleteTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        user_with_publication = User.objects.create(
            email="user_with_publication@test.com",
            password="test_pass2",
            is_active=True,
        )
        redactor_user = User.objects.create(
            email="redactor_user@test.com", password="test_pass2", is_active=True
        )
        admin_user = User.objects.create(
            email="admin_user@test.com", password="test_pass2", is_active=True
        )

        call_command("create_default_groups")

    def setUp(self):
        self.user_with_publication = User.objects.get(
            email="user_with_publication@test.com"
        )
    ...
    def create_posts(self):
        posts = Posts.objects.get_or_create(
            content="PUBLISHED posts by activated user",
            title="test from activated user",
            author=self.user_with_publication,
            status="PUBLISHED",
            id=1,
        )
...
class PostsUpdateTest(TestCase):
    ...
    def test_update_from_not_authorized_user(self):
        resp = self.client.post(
            reverse("posts:update", kwargs={"pk": self.post.id}),
            {"title": "updated_test_status", "content": "sdfsdfsdcscs"},
        )
        self.assertEquals(resp.status_code, 403)
    ...
    def test_delete_not_exiting_posts(self):
        self.client.force_login(self.admin_user)
        resp = self.client.post(
            reverse("posts:update", kwargs={"pk": 155}),
            {"title": "updated_test_status", "content": "sdfsdfsdcscs"},
        )
        self.assertEquals(resp.status_code, 404)
